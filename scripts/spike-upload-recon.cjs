...
async function inspectEditSubmission(page, assignment) {
  ...
  const meta = await page.evaluate(() => {
    ...
    // Restricciones del filemanager. Moodle las expone como data-* en el div
    // contenedor Y como texto legible al lado del uploader — tomo ambos.
    const filemanager = main.querySelector(
      '.filemanager, [id^="filemanager-"], [data-fieldtype="filemanager"]',
    );
    const filemanagerAttrs = filemanager
      ? Object.fromEntries(
          Array.from(filemanager.attributes)
            .filter((a) => a.name.startsWith('data-'))
            .map((a) => [a.name, a.value]),
        )
      : null;

    // "Tamaño máximo para archivos nuevos" / "Número máximo de archivos" /
    // "Tipos de archivo aceptados" viven en un dl/dt/dd al lado del uploader.
    const restrictionText = normalize(
      main.querySelector('.fitem_ffilemanager, [data-groupname="fgroup_id_files"]')?.textContent ||
        '',
    );

    const acceptedTypes = Array.from(
      main.querySelectorAll('.filepicker-accepted-types, .fp-accepted_types, .form-inline .badge'),
    )
      .map((el) => normalize(el.textContent))
      .filter(Boolean);

    // Onlinetext — si el submission acepta texto además de/en vez de archivo.
    const onlineText = main.querySelector(
      'textarea[name*="onlinetext"], .assignsubmission_onlinetext textarea',
    );

    // Declaración obligatoria (checkbox "Esta entrega es mi propio trabajo").
    const declaration = Array.from(main.querySelectorAll('input[type="checkbox"]')).map((el) => ({
      name: el.name,
      id: el.id,
      label: normalize(el.closest('label, .form-check')?.textContent || el.parentElement?.textContent),
    }));

    // Botones del form. Los enumero para VER cuáles hay — nunca los clickeo.
    ...
    // Archivos que ya están en el draft (si el alumno subió y guardó como draft).
    const existingDraftFiles = Array.from(
      main.querySelectorAll('.filemanager .fp-filename-icon, .fm-content-wrapper .fp-filename'),
    )
      .map((el) => normalize(el.textContent))
      .filter(Boolean);
